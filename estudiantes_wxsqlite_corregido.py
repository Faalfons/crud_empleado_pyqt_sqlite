    ...
    def actualizar_estudiante(self, event):
        ...
        if documento and nombre and fecha and correo:
            try:
                conexion = sqlite3.connect("uninproyectos.db")
                cursor = conexion.cursor()
                cursor.execute("SELECT * FROM estudiantes WHERE documento=?", (documento,))
                if cursor.fetchone():
                    cursor.execute("UPDATE estudiantes SET nombre=?, fecha=?, correo=? WHERE documento=?",
                    ...
                    wx.MessageBox("No se encontró un estudiante con ese documento.", "Error", wx.OK | wx.ICON_ERROR)
                conexion.close()
            except sqlite3.IntegrityError:
                wx.MessageBox("El correo ya está registrado a otro estudiante.", "Error", wx.OK | wx.ICON_ERROR)
        else:
            wx.MessageBox("Todos los campos son obligatorios.", "Advertencia", wx.OK | wx.ICON_WARNING)
